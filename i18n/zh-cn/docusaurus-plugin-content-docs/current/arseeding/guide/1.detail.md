...
- `payCurrencyTag`: 指定支付费用的 token tag, 支持使用 everPay 上的任意 token 进行支付, 可通过 `getTokenTagByEver('symbol')`,获取 symbol 所有 tag，。
...
const privateKeyJSON = './test_key.json'
...
- `order`: 为上传数据返回的 order, 注意: order 中必需包含 `tag` 字段，若没有 tag 字段，可通过 `currency` 查询 everpay 支持 `currency tag`。
